...
int main() {
  ...
  for (int col = 0; col < 11; col++) lanc.porcent[col] = lanc.soma[col] / TOTAL;
  ...
  for (int item = 0; item < 3; item++) {
    if (item == 0) {
      printf("Soma:\t");
      for (int col = 0; col < 11; col++) printf("%d\t", lanc.val[col]);
      putchar('\n');
    }  // Fim if (item==0)
    else if (item == 1) {
      printf("Qtdade:\t");
      for (int col = 0; col < 11; col++) printf("%d\t", lanc.soma[col]);
      putchar('\n');
    }  // Fim if (item==1)
    else {
      printf("%%:\t");
      for (int col = 0; col < 11; col++) printf("%.2f\t", lanc.porcent[col]);
      putchar('\n');
