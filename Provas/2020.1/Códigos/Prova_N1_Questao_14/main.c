...
int main() {
  ...
  for (int i = 0; i < LIN; i++)
    for (int j = 0; j < COL; j++) temperatura[i][j] = j - AUX;
  ...
  for (int j = 0; j < COL; j++) {
    ...
    scanf("%d", &temp);
    for (int j = 0; j < COL; j++)
      if (temperatura[0][j] == latitude) temperatura[1][j] = temp;
    fflush(stdin);
  ...
  for (int j = (COL / 2); j > 0; j--) {
    if ((temperatura[1][-j + COL / 2] > temperatura[1][j + COL / 2]) &&
        (temperatura[1][-j + COL / 2] != -temperatura[1][j + COL / 2]))
      printf("Lat. %d do Sul eh mais quente do que lat. %d do Norte!\n",
             temperatura[0][-j + COL / 2], temperatura[0][j + COL / 2]);

    else if ((temperatura[1][-j + COL / 2] < temperatura[1][j + COL / 2]) &&
             (temperatura[1][-j + COL / 2] != -temperatura[1][j + COL / 2]))
